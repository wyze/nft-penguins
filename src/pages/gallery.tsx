...
type Metadata = {
  attributes: Array<Record<'value', string>>
  description: string
  image: string
  name: string
}
...
export default function Gallery() {
  ...
  const current = useQuery(
    'current',
    async () => {
      const totalSupply = await getContract().totalSupply()
  ...
  const tokenUris = useQuery(
    ['token-uris', current.data],
    () => {
  ...
  const tokensForAccount = useQuery(
    ['tokens', account],
    async () => {
      const contract = getContract()
  ...
  const uris = useMemo(
    () =>
      tokenUris.data
        ?.map((uri, index) => [uri, index] as const)
        .filter((_, index) =>
          checked ? tokensForAccount.data?.includes(index) : true
        ),
    [checked, tokenUris.data, tokensForAccount.data]
  )
  ...
  return (
    <>
      <FormControl
        ...
        mb="10"
      >
        <FormLabel htmlFor="only-mine" mb="1">
          Show only mine
        </FormLabel>
        <Switch
          id="only-mine"
      ...
      <SimpleGrid columns={3} gap={5} justifyContent="center">
        {uris?.map(([uri, index]) => (
          <Box key={index}>
            <Flex align="center" justify="space-between">
              <Text fontWeight="bold">Penguins #{index}</Text>
              <Link
                href={`https://testnets.opensea.io/assets/${address}/${index}`}
                isExternal
              >
                <OpenSeaIcon />
              </Link>
            </Flex>
            <Box borderRadius="md" h="215" overflow="hidden" w="215px">
              <Image
                alt={`Penguins #${index}`}
                height={215}
                key={index}
